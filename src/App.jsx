            ...
            <div style={{ width: "100%", maxWidth: 500, marginTop: 56 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                {[
                  ...
                  <div key={tier.price} style={{ background: tier.highlight ? BD : "rgba(255,255,255,0.02)", border: `1px solid ${tier.highlight ? BB : "rgba(255,255,255,0.06)"}`, borderRadius: 18, padding: "22px 20px", textAlign: "left" }}>

                    <div style={{ fontSize: 30, fontWeight: 700, color: tier.highlight ? BL : "#E4EEFF", marginBottom: 4 }}>{tier.price}</div>
                    <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 14, color: tier.highlight ? BL : "rgba(228,238,255,0.6)" }}>{tier.title}</div>
                    {tier.perks.map(p => (
        ...
        <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", position: "relative", zIndex: 1 }}>
          <div style={{ maxWidth: 680, width: "100%" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 18 }}>
              {/* Basic */}
              <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: "24px", display: "flex", flexDirection: "column" }}>
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.68)", marginBottom: 6 }}>Basic Results</div>
                <div style={{ fontSize: 34, fontWeight: 700, color: "#FFFFFF", marginBottom: 4 }}>$1.99</div>
                ...
                <button onClick={() => !paymentLoading && handlePurchase("basic")}
                  style={{ background: "rgba(75,127,245,0.15)", border: `1px solid ${BB}`, color: BL, padding: "13px", borderRadius: 100, fontSize: 14, fontWeight: 700, cursor: paymentLoading ? "not-allowed" : "pointer", opacity: paymentLoading ? 0.6 : 1, transition: "all 0.2s" }}
                  onMouseOver={e => { if (!paymentLoading) e.currentTarget.style.background = BD; }}
                  onMouseOut={e => { e.currentTarget.style.background = "rgba(75,127,245,0.15)"; }}>
